...
from ..utils import solve_time_frac_parabolic_equation, solve_dim_frac_parabolic_equation

router = APIRouter(prefix="/api/frac_parabolic1d", tags=["frac_parabolic1d"])

class MethodType(str, Enum):
    time_frac = 'solve_time_frac_parabolic_equation'
    dim_frac = 'solve_dim_frac_parabolic_equation'
...
def solve(req: SolveRequest):
    ...
    try:
        ...
        if req.method == MethodType.time_frac:
            x, u = solve_time_frac_parabolic_equation(req.T, req.L, req.N, req.M, req.a, req.alpha, init_callable, left_callable, right_callable, f_callable)
        else:
            x, u = solve_dim_frac_parabolic_equation(req.T, req.L, req.N, req.M, req.a, req.alpha, init_callable, left_callable, right_callable, f_callable)
